...
recomputed after style updates.
...
After a style update, invalidation decides which work can be reused
...
by generating style updates that should force the browser to
recompute affected rendering state. Its central strategy is
self-differential testing: the system compares the browser's incremental
layout version against its own refreshed layout version for the same final
page state. This allows \toolname to detect stale layout state without
...
\begin{itemize}
    ...
    style updates, compares incremental layout versions and refreshed layout versions,
    ...
    artifacts, test cases, and benchmarks at the following
    URL: XXX.
\end{itemize}
